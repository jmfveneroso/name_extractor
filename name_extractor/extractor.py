...
class Extractor():
  ...
  the most likely sequence of labels for a sequence of tokens given
  ...
  def __init__(self):
    ...
    # Feature probabilities will be calculated on a second passing over the
  ...
  def load_conditional_probs(self, directory = "../probabilities"):
    ...
    with open(os.path.join(directory, "cond_name_probs.txt")) as f:
      for line in f:
        ...
        self.cond_name_probs[word] = float(prob.strip())
  ...
  def get_sequence_probs(self, tkns, second_passing):
    """ 
