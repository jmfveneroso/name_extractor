...
invaluable data about the academic environment. However, researcher
affiliation information is frequently missing or outdated.
We propose a method of researcher name extraction from faculty directories
to keep up to date information about researcher 
...
web pages from multiple universities across the world in various languages 
...
probabilities for the next token's label. Put in other words, $ P(y_i|t_{i-1} \neq Name) \approx P(y_i) $.
...
that name starting from the first one and slide the window right by the number of name tokens. It is
important that the window is at least the size of the biggest possible name (5 seems to be enough on most 
...
$ \{W, N_1, N_1, N_2\} $, $\{W, N_1, N_2, N_2\} $, $ \ldots $. 
...
point has shown good results. For a window of size 4, we would need to estimate prior 
...
$ P(t_i|W) $ probabilities were estimated over a corpus of a hundred thousand documents
obtained during the crawling stage. In the latter case all names were removed from the 
corpus in order to keep only non-name tokens.
...
clues from context words and the current token value. Structural features 
infer token probabilities based on the surrounding HTML structure
such as tag names and nesting depth. 
...
so we cannot extract useful probabilities looking through the entire collection. 
...
street names that could be confused for a person's name. They involved looking at 
context words, word capitalization, honorifics, etc. However, these trials
were not very productive.

By design, the features were chosen with improvements to recall rather than precision in mind. Because
...
significant results are boldened. The base model uses only priors with the details already described 
...
All measures were obtained by the averaged results of a 5 fold cross validation run. 
...
Table \ref{tab:ne_plus} describes the results for the best model 
(features 3, 5 and 7) running secondary estimates 3, 4, 5 and 6 times.

Our base model already achieves good results (91.71\% precision, 91.39\% recall,
and 0.9155 F-measure), because it relies on simple but universal probabilities
without making assumptions about data organization or contextual information.

The secondary estimates strategy was very succesful in improving all metrics by
incorporating structural features on the base model. The best model, achieved
a precision of 94.23\%, recall of 97.14\% and F-measure 0.9566 with statistical
significant improvement in both recall and F-measure.

Finally, we observe that multiple runs on the secondary estimates strategy do not
increase performance significantly. In fact, it reaches a plateau after only 3 to 4
runs. Further runs were omitted because there were no further changes after 6 runs.
...
The secondary estimates strategy was very succesful in further improving the
base model. It can also be remodeled to fit other statistical classifiers since
it doesn't rely on any particular implementation details of our strategy.
