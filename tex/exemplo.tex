...
Let $ T = (t_1, t_2, ..., t_n) $ be a sequence of tokens, and $ Y = (y_i, y_{i + 1}, ..., y_n) $ 
a sequence of labels where $ y_i = N $ means token $ i $ is a name and $ y_i = W $ means token 
$ i $ is  a word (not a name). Then, we are interested in estimating the sequence of labels $ Y $ 
with maximum probability for a given sequence of tokens:

\[

\argmax_Y P(t_i = y_i, t_{i+1} = y_{i+1}, ..., t_n = y_n)

\]

where $ P(x_i = Y_i) $ means the probability of $ x_i $ having label $ y_i $.
We may employ the probability chain rule to better estimate the joint probability
such as in a regular n-gram model. For a bigram model for example we would have:

\[

P(y_i|x_{i-1} = y_{i-1}, x_{i-2} = y_{i-2}, ... x_1 = y_1) = P(y_i|x_{i-1} = y_{i-1})

\]

At this point we make an assumption that the probability of token $ i $ having label
$ Y_i $ depends on the labels, however it is independent of the particular values
of the previous tokens. For example, given a sequence of tokens $ (John, Hall) $ then 
the conditional probability $ P(Hall|John = name) = P(Hall|any name) $. In other words, 
this means that the probability of "Hall" being a last name is the same regardless of a 
person's first name, as long as we are sure the previous token is a name. So we can
write the conditional probability as:

\[

P(t_i = y_i|t_{i-1} = y_{i-1}, t_{i-2} = t_{i-2}, ... t_1 = y_1) = P(t_i = y_i|y_{i-1}, y_{i-2}, ... y_1)

\]

Where $ P(y_i) $ means the unconditional probability of any token having label $ y_i $.
And the joint probability then becomes:

\[

P(t_i = y_i, t_{i+1} = y_{i+1}, ..., t_n = y_n) = P(t_i = y_i) P(t_{i+1} = y_{i+1}|y_i) ... P(t_n = y_n|y_i, y_{i+1}, ...)

\]

Considering that $ P(t_i = y_i) $ is just a notation for $ P(t_i, y_i) $ and
given that $ P(t_i, y_i) = P(t_i|y_i)P(y_i) $ we can write the joint probability as:

\[

P(t_i = y_i, t_{i+1} = y_{i+1}, ..., t_n = y_n) = P(t_i|y_i)P(y_i) x P(t_{i+1}|y_{i+1}, y_i)P(y_{i+1}|y_i) ... P(t_n|y_n)P(y_n|y_i, y_{i+1}, ...)
P(t_i = y_i, t_{i+1} = y_{i+1}, ..., t_n = y_n) = P(t_i|y_i)P(t_{i+1}|y_{i+1}, y_i)... P(t_n|y_n,...) P(y_i, y_{i+1}, ..., y_n)

\]

Approximating $ P(t_i|y_i, y_{i-1}, ...) $ by $ P(t_i|y_i) $ we get

\[

P(t_i = y_i, t_{i+1} = y_{i+1}, ..., t_n = y_n) = P(t_i|y_i)P(t_{i+1}|y_{i+1})... P(t_n|y_n) P(y_i, y_{i+1}, ..., y_n)

\]

This is out basic formula. We can easily obtain estimates for $ P(t_i|y_i) $. The probabilities
were estimated $ P(token|name) $ with the DBLP author names, which amount to aproximately 1.5 million
names. The probabilities $ P(token|not name) $ were estimated from the corpus built from university
websites, as a heuristic to remove all names, all captalized words were ignored.

The probabilities $ P(y_i, y_i+1, ..., y_n) $ were estimated on the labeled examples. All names
were tagged manually. In this case it is necessary to estimate all possible combinations
of words and names for a given window size. 
...
Most of the time, names are wholy contained inside a single html element, eventhough this is
not always the case. So it is useful to incorporate this evidence in our formula to delimit 
name boundaries. For example, the probability P(NNWW) becomes higher when the second element
is inside a different element than the third, this evidence will be presented in the following
format P(NN*WW). So we need to estimate all sets of labels for each breaking point.

We may also incorporate other sources of evidence in the single token probabilities 
P(t|N) and P(t|W). Some useful features are token length and token incidence.

Second passing:

Faculty repositories usually have names disposed in an ordery fashion. That means they share
similar attributes. If our basic model was able to find a relevant part of the names on the
first passing, we may improve its precision by estimating some structural attributes on the
returned collection of names and then use those estimates to improve individual token 
probabilities.

The following features were tested in the second passing: 
first and second parent, third parent, element position, class name, element depth.
