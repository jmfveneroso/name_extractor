...
they take a rather relaxed approach in regard to formal structure. Hierarchy, element disposition,
...
but are in no way subject to any strict rule.
...
probabilistic method we hope to build a general approach to solve this problem and handle the name 
extraction problem in general.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{pics/jap_intelligent_robotics_lab}
  \caption{Example of faculty directory}
...
State-of-the-art named entity recognition approaches like conditional random fields do not 
...
the mistakes of other classifiers. The base model has already achieved recall and precision rates 
above 90\%, but the performance can be increased even further by incorporating HTML structural
features to help estimating probabilities.
